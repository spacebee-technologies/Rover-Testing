...
/*=====================[Variables]================================*/
    typedef enum
    {
        ...
        CANopen_STOPPED,                    //Estado parado, deja inactivos todos los objetos de comunicaci贸n posibles (no se pueden enviar ni recibir PDOs ni SDOs), excepto a los comandos NMT que pueden cambiar a cualquiera de los estados restantes y Heartbeat. Es decir, un nodo solo puede realizar vigilancia de nodos o latidos, pero no puede recibir ni transmitir mensajes.
    } CANopen_STATES;                       //Enumaracion de los estados posibles de la maquina de estado CANopen

    volatile static CANopen_STATES state = CANopen_INITIALIZATION; //Variable para guardar el estado de la aplicaci脙鲁n
...
uint8_t Verify_writing_by_can(void){
    volatile static CAN_ESTADO estado = CAN_LIBRE;         //Variable para guardar el estado de la aplicaci贸n CAN
...
/*========================================================================
  ...
  Parametro de entrada:  uint32_t *messageID        = Puntero a la variable a donde se quiere guardar el id
                         ...
                         uint8_t *messageLength     = Puntero a la variable a donde se quiere guardar el tama帽o del mensaje
  No retorna nada
  ========================================================================*/
